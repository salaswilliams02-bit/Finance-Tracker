...
const DEFAULT_CATEGORIES = [
  'Groceries','Dining Out','Rent/Mortgage','Utilities','Transportation','Health & Fitness','Entertainment','Shopping','Travel','Other',
]
...
export default function App(){
  ...
  const totals = useMemo(()=>{
    ...
    const byMonth = [...byMonthMap.entries()].filter(([ym])=>ym).sort().map(([ym,value])=>({ ym, value }))
    return { total, byCat, byMonth }
  }, [filteredTx, transactions])

  const monthlyBudgetTarget = useMemo(()=> goals.reduce((s,g)=> s + (g.target||0)/12, 0), [goals])

  const addTransaction = (tx)=> setTransactions(prev => [{ id: crypto.randomUUID(), ...tx }, ...prev])
  ...
  const importCSV = (text)=>{
    ...
    const hasHeader = ['date','description','amount','category'].every(h=>header.includes(h))
  ...
  const exportCSV = ()=>{
    ...
    const rows = transactions.map(t => [t.date, cleanCSV(t.description), t.amount, cleanCSV(t.category)].join(','))
  ...
  const resetAll = ()=>{
    if (confirm('This will delete all transactions, goals, and categories. Continue?')){
      setTransactions([]); setGoals([]); setCategories(DEFAULT_CATEGORIES)
    }
  }
      ...
      <header className="sticky top-0 z-10 backdrop-blur bg-white/70 border-b">
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center gap-3">
          ...
          <div className="ml-auto flex gap-2">
            <button onClick={exportCSV} className="px-3 py-1.5 rounded-lg border hover:bg-gray-100 flex items-center gap-1"><Download className="w-4 h-4"/> Export</button>
            <ImportCSV onImport={importCSV}/>
            <button onClick={resetAll} className="px-3 py-1.5 rounded-lg border hover:bg-red-50 text-red-600 flex items-center gap-1"><Trash2 className="w-4 h-4"/> Reset</button>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-6xl mx-auto px-4 py-6 grid gap-6">
        <section className="grid lg:grid-cols-3 gap-3">
          <div className="p-4 bg-white rounded-2xl shadow-sm border">
            ...
            <input type="month" value={filterMonth} onChange={e=>setFilterMonth(e.target.value)} className="mt-1 w-full border rounded-xl px-3 py-2"/>
          </div>
          <div className="p-4 bg-white rounded-2xl shadow-sm border">
            <label className="block text-sm text-gray-500">Category</label>
            <select value={filterCategory} onChange={e=>setFilterCategory(e.target.value)} className="mt-1 w-full border rounded-xl px-3 py-2">
              <option>All</option>
          ...
          <div className="p-4 bg-white rounded-2xl shadow-sm border flex items-center justify-between">
            ...
            <div className="text-right">
              ...
              <p className={`text-2xl font-semibold ${monthSpend > monthlyBudgetTarget ? 'text-red-600' : 'text-emerald-600'}`}>{currency.format(monthlyBudgetTarget || 0)}</p>
            </div>
          </div>
        </section>
        ...
        <section className="grid md:grid-cols-3 gap-6">
          <div className="md:col-span-2 p-4 bg-white rounded-2xl shadow-sm border">
            ...
            <TransactionForm onAdd={addTransaction} categories={categories} onAddCategory={addCategory}/>
          </div>
          <div className="p-4 bg-white rounded-2xl shadow-sm border">
            <h2 className="font-medium mb-3 flex items-center gap-2"><TrendingUp className="w-5 h-5"/>Add Goal</h2>
        ...
        <section className="grid md:grid-cols-3 gap-6">
          <Card title="Category Breakdown">
            ...
            <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-sm mt-2">
              {totals.byCat.map((c, i) => (
                <div key={i} className="flex items-center justify-between"><span>{c.name}</span><span className="font-medium">{currency.format(c.value)}</span></div>
              ))}
            </div>
          </Card>
          ...
          <Card title="Goals Progress">
            <div className="space-y-3">
              {goals.length === 0 && <p className="text-sm text-gray-500">No goals yet. Add one on the right.</p>}
              {goals.map(g => {
                ...
                return (
                  <div key={g.id} className="p-3 rounded-xl border">
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <button onClick={()=>removeGoal(g.id)} className="text-red-600 hover:underline text-sm">Remove</button>
                    </div>
                    <div className="text-sm text-gray-600 mb-1 flex justify-between">
                      <span>{currency.format(g.current||0)} / {currency.format(g.target||0)}</span>
              ...
              <tbody>
                {filteredTx.map(t => (
                  <tr key={t.id} className="border-t">
                    ...
                    <td className="py-2 pr-3 text-right font-medium">{currency.format(t.amount)}</td>
                    <td className="py-2 pr-3 text-right"><button onClick={()=>removeTransaction(t.id)} className="text-red-600 hover:underline">Delete</button></td>
                  </tr>
                ))}
              </tbody>
            </table>
  ...
  return (
    <form onSubmit={submit} className="grid md:grid-cols-5 gap-3 items-end">
      ...
      <div className="md:col-span-2">
        ...
        <input value={description} onChange={e=>setDescription(e.target.value)} placeholder="e.g., Whole Foods" className="mt-1 w-full border rounded-xl px-3 py-2"/>
      </div>
      <div>
        <label className="block text-sm text-gray-500">Amount</label>
        <input type="number" step="0.01" min="0" value={amount} onChange={e=>setAmount(e.target.value)} placeholder="0.00" className="mt-1 w-full border rounded-xl px-3 py-2"/>
      </div>
      <div>
        <label className="block text-sm text-gray-500">Category</label>
        <div className="flex gap-2 mt-1">
          <select value={category} onChange={e=>setCategory(e.target.value)} className="flex-1 border rounded-xl px-3 py-2">
            {categories.map(c => <option key={c}>{c}</option>)}
          </select>
        </div>
      </div>
      ...
      <div className="md:col-span-5 grid md:grid-cols-3 gap-3 items-end">
        <div className="md:col-span-2">
          ...
          <input value={newCategory} onChange={e=>setNewCategory(e.target.value)} placeholder="e.g., Subscriptions" className="mt-1 w-full border rounded-xl px-3 py-2"/>
        </div>
        <button type="button" onClick={addCat} className="px-3 py-2 rounded-xl border hover:bg-gray-100 flex items-center gap-2 justify-center"><PlusCircle className="w-4 h-4"/> Add Category</button>
      </div>

      <div className="md:col-span-5 flex justify-end">
        <button type="submit" className="px-4 py-2 rounded-xl bg-emerald-600 text-white hover:brightness-110">Add Transaction</button>
      </div>
    </form>
  )
}
...
function GoalForm({ onAdd }){
  ...
  const submit = (e)=>{
    ...
    setName(''); setTarget(''); setCurrent(''); setDue('')
  }
  ...
  return (
    <form onSubmit={submit} className="grid gap-3">
      <div>
        ...
        <input value={name} onChange={e=>setName(e.target.value)} placeholder="e.g., Emergency Fund" className="mt-1 w-full border rounded-xl px-3 py-2"/>
      </div>
      <div>
        <label className="block text-sm text-gray-500">Target Amount</label>
        <input type="number" step="0.01" min="0" value={target} onChange={e=>setTarget(e.target.value)} placeholder="0.00" className="mt-1 w-full border rounded-xl px-3 py-2"/>
      </div>
      <div>
        <label className="block text-sm text-gray-500">Current Saved</label>
        <input type="number" step="0.01" min="0" value={current} onChange={e=>setCurrent(e.target.value)} placeholder="0.00" className="mt-1 w-full border rounded-xl px-3 py-2"/>
      </div>
      <div>
        <label className="block text-sm text-gray-500">Due (optional)</label>
        <input type="month" value={due} onChange={e=>setDue(e.target.value)} className="mt-1 w-full border rounded-xl px-3 py-2"/>
      </div>
      <button className="px-4 py-2 rounded-xl bg-indigo-600 text-white hover:brightness-110">Add Goal</button>
    </form>
  )
}
